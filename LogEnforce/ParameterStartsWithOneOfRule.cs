...
{
    public class ParameterStartsWithOneOfRule : Rule
    {
        ...
        public ParameterStartsWithOneOfRule(XmlNode node)
            : base(node)
        {
        ...
        public override void Enforce(LAS_File las)
        {
            ...
            {
                this.Status = "Warning";
                this.Comment = "Parameter " + Constant + " is set to [" + lc + "] but should start with [" + ret + "].";
                return;
            }
            this.Status = Severity;
            StringBuilder sb = new StringBuilder();
            sb.Append("Parameter " + Constant + " is set to [" + lc + "] but should start with one of [");
        ...
        private string FindValueUpper(string val)
        {
            foreach (string s in Values)
            {
                if (val.StartsWith(s.ToUpper())) return s;
            }
            return "";
        }
    }
