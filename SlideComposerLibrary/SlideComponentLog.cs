...
namespace Petronode.SlideComposerLibrary
{
    ...
    /// </summary>
    public class SlideComponentLog : SlideComponent
    {
        /// <summary>
        ...
        public SlideComponentLog(Slide Parent, string[] args)
            : base(Parent, args) 
        {
            if(args.Length > 6) Name_y = args[6].Trim();
            if(args.Length > 7) Name_x = args[7].Trim();
            if (args.Length > 8) LineType = args[8].Trim().ToLower();
        ...
        public override string[] AddAttribute(string attribute)
        {
            ...
            try
            {
                switch (attrib[0])
                {
                    ...
                    case "ltype": LineType = attrib[1].Trim(); break;
        ...
        protected override void RecoverAttributes()
        {
            try
            {
                ...
                this.LineType = ParentSlide.DefaultParameters["ltype"];
        ...
        protected override void SaveAttributes()
        {
            try
            {
                ...
                ParentSlide.DefaultParameters["ltype"] = LineType;
        ...
        private void plotPointScaled(Graphics g, Pen pen, List<PointF> points, double xV, double yV)
        {
            ...
            if (!double.IsNaN(xV) && !double.IsNaN(yV))
            {
                ...
                points.Add( new PointF(xf, yf));
                return;
            }
            
            // Point is a missing value
            if (points.Count <= 0) return;

            // If a single point - no line possible
            if (points.Count > 1) g.DrawLines( pen, points.ToArray());

            // Clear array - section completed
            points.Clear();
        }
